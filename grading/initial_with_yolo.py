...
import re
import json
# Enable YOLO AI Model with GPU acceleration
YOLO_AVAILABLE = True
...
# YOLO CONFIGURATION
# =========================================================
YOLO_MODEL_PATH = r"D:\yolo_cls\runs\detect\train11\weights\best.pt"  # Trained YOLO detection/classification model
GOOD_CLASS_NAMES = ['good', 'Good'] 
    ...
    def __init__(self, model_path):
        ...
        if YOLO_AVAILABLE:
            ...
            if not os.path.exists(onnx_path) and os.path.exists(model_path):
                try:
                    print(f"[YOLO GPU] Exporting {model_path} to ONNX for GPU acceleration...")
                    yolo_model = YOLO(model_path)
                    yolo_model.export(format='onnx', imgsz=640, dynamic=True)
                except Exception as e:
            ...
            if os.path.exists(onnx_path):
                try:
                    ...
                    # Enable CUDAExecutionProvider for RTX GPU acceleration
                    self.session = ort.InferenceSession(onnx_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
                    print(f"[YOLO GPU] Loaded ONNX model on GPU! Providers: {self.session.get_providers()}")
                except Exception as e:
                    print(f"[YOLO ONNX ERROR] {e}")

            if self.session is None and os.path.exists(model_path):
                try:
                    self.model = YOLO(model_path)
                    print(f"[YOLO PyTorch] Loaded PyTorch model: {model_path}")
                except Exception as e:
                    print(f"[YOLO PyTorch ERROR] {e}")

    def get_cashew_categories_batch(self, crops):
        """
        Run batch inference on cashew crops using GPU (ONNX CUDA / PyTorch GPU).
        ...
        # 1. GPU ONNX Inference (Blazing fast, ~8ms per crop)
        if self.session is not None:
            try:
                imgs = [cv2.resize(c, (640, 640)).transpose(2,0,1) for c in crops]
                ...
                for i in range(len(crops)):
                    ...
                    top_idx = np.argmax(best_conf)
                    conf = float(best_conf[top_idx])
                    cls_id = int(best_cls[top_idx])
                    cls_name = self.class_names.get(cls_id, 'good')
                    batch_results.append((cls_name, conf))
                    
                ai_time = (time.time() - start_ai) * 1000
                print(f"[AI GPU CORE] Processed {len(crops)} cashews on GPU in {ai_time:.1f}ms ({(ai_time/len(crops)):.1f}ms/crop)")
                return batch_results
            except Exception as e:
                print(f"    [YOLO ONNX GPU ERROR] {e}")
        ...
        if self.model is not None:
            try:
                ...
                results = self.model(crops, verbose=False, conf=YOLO_CONF_THRESHOLD, device=dev, imgsz=224)
                batch_results = []
                for result in results:
                    if len(result.boxes) > 0:
                        best_box = result.boxes[0]
                        cls_name = self.model.names[int(best_box.cls[0])].lower()
                        batch_results.append((cls_name, float(best_box.conf[0])))
                    else:
            ...
            for idx, (obj_id, obj_info, true_exit_time, time_overshoot) in enumerate(disappeared_objs):
                ...
                if yolo_cat:
                    if yolo_cat.lower() in [name.lower() for name in GOOD_CLASS_NAMES]:
                        ...
                        # YOLO Model detected defect (bad, blackdot, brown, multi, oilly, unpill)
                        final_grade = yolo_cat.lower()
                        
                # OpenCV Backup heuristics (if YOLO prediction was missing)
                if not final_grade:
