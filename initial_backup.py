...
MIN_CASHEW_AREA = 3500       # Increased to 3500 to ignore dots/noise
        ...
        try:
            ...
            for result in results:
                if len(result.boxes) > 0:
                    # Look for ANY 'good' box first
                    for box in result.boxes:
                        cls_id = int(box.cls[0])
                        conf = float(box.conf[0])
                        cls_name = self.model.names[cls_id].lower()
                        # print(f"    [YOLO DETECT] ID:{getattr(crop, 'obj_id', '?')} → {cls_name} ({conf:.2f})")
                        
                        if cls_name in [name.lower() for name in GOOD_CLASS_NAMES]:
                            return cls_name, conf
                            
                    # If no 'good' box, return the highest confidence box
                    box = result.boxes[0]
                    cls_id = int(box.cls[0])
                    conf = float(box.conf[0])
                    cls_name = self.model.names[cls_id].lower()
...
class ZoneProcessor:
    ...
    def process_frame(self, frame, quality_filter=None):
        ...
        # Step 3: Morphological refinement with ELLIPTICAL kernels (smoother than rectangular)
        ...
        # Step 4: ULTRA-SMOOTH EDGES - Heavy Gaussian blur (21x21) on binary mask
        # This is the KEY trick: blurring binary mask + re-threshold at 127
        # creates perfectly smooth contour boundaries without jagged edges
        ...
        for i, c in enumerate(adjusted_contours):
            ...
            # --- CASHEW REALITY CHECK (DENSITY VALIDATION) ---
            # Create a mask of just this object and check cashew color content
            ...
            # If the shape is <15% cashew color, it's just belt grain/noise
            if density < 0.15:
                continue
            
            # --- ROLLER ELIMINATION FILTERS ---
            rect = cv2.minAreaRect(c)
            (w_p, h_p) = rect[1]
            mm_size = max(w_p, h_p) * PIXEL_TO_MM_RATIO
            # Aspect ratio: max/min ensures it works for both vertical and horizontal orientations
            aspect_ratio = max(w_p, h_p) / max(1, min(w_p, h_p))
            
            # 1. Size filter (Reject rollers)
            if mm_size > MAX_CASHEW_MM:
                # print(f"  [FILTER] Rejecting large object: {mm_size:.1f}mm")
                continue
                
            # 2. Shape filter (Reject rollers/lines)
            if aspect_ratio > MAX_ASPECT_RATIO:
                continue

            # 3. Solidity Check (REJECT DOTS/NOISE)
            # Area / (Bounding Box Area). Real cashews are solid (>0.5), noise is scattered.
            solidity = area / max(1, w_p * h_p)
            if solidity < 0.50:
                continue

            # 3. Micro-Reflection Filter (Reject anything too narrow or short)
            if min(w_p, h_p) < 20:
                # print(f"  [FILTER] Rejecting micro-object: {min(w_p, h_p):.1f}px")
                continue

            # DEFECT CATEGORY PRIORITIES
            current_grade = None
            is_good = True
            
            # Extract crop for specialized checks (SQUARE CROPPING)
            ...
            if crop.size > 0:
                # --- 1. YOLO CLASSIFICATION (PRIORITY) ---
                ...
                if quality_filter and quality_filter.model:
                    ...
                    if yolo_cat:
                        if yolo_cat in [name.lower() for name in GOOD_CLASS_NAMES]:
                            # Only trust AI Good if confidence is VERY high (From Control Panel)
                            if yolo_conf > YOLO_STRICT_BYPASS:
                                current_grade = None 
                                is_good = True
                                yolo_confirmed_good = True
                        else:
                            # YOLO detected a specific defect class
                            current_grade = yolo_cat
                            is_good = False
                
                # --- 2. HEURISTIC BACKUPS (Safely re-enabled) ---
                if not current_grade and not yolo_confirmed_good:
                    ...
                    # 2. RGB DEFECT CHECK (Shell, Orange, Color)
        ...
        for obj_id in disappeared_ids:
            ...
            if obj_info:
                ...
                # --- LOGGING FILTER: Only process real cashews (> 15mm) and tracked for >= 2 frames ---
                if max_mm >= 15.0 and len(obj_info.get('measurements', [])) >= 2:
                    # Resolve final grade with CONSENSUS (Temporal Majority Voting)
                    history = obj_info.get('grade_history', [])
                    last_crop = obj_info.get('last_crop')
                    
                    # Filter out None (None represents a 'good' frame)
                    defect_history = [g for g in history if g is not None]
                    
                    final_grade = None
                    if defect_history:
                        # Consensus: A defect must appear in at least 30% of processed frames to be accepted
                        counts = Counter(defect_history)
                        most_common, count = counts.most_common(1)[0]
                        if count >= max(1, len(history) // 3):
                            final_grade = most_common
                    
                    # If no consensus defect, use size-based grading
                    if not final_grade:
                    ...
                    if last_crop is not None:
                        try:
                            ...
                            if not contour_drawn:
                                # Fallback: re-detect on crop using bilateral+Otsu+heavy blur
                                smooth_s = cv2.bilateralFilter(save_img, 11, 80, 80)
    ...
    try:
        while True:
            ...
            for processor in zone_processors:
                ...
                # NOTE: Contour drawing is handled by draw_zone() above
                # Do NOT draw contours here again (causes double border)
